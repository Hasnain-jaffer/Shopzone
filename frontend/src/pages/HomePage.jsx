...
const shopCategories = [
  { name: "Electronics & Gadgets", icon: "📱", bg: "#e0e7ff" },   // brand-100
  { name: "Fashion & Apparel", icon: "👗", bg: "#f3e8ff" },        // brand-accent tint
  { name: "Home & Kitchen", icon: "🏠", bg: "#dbeafe" },           // cool complement
  { name: "Health & Fitness", icon: "💪", bg: "#eef2ff" },         // brand-50
  { name: "Beauty & Personal Care", icon: "💄", bg: "#ede9fe" },   // violet, between brand + accent
];
...
const HomePage = () => {
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      {/* ── HERO ── */}
      <div
        className="px-8 md:px-16 py-12 grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-ink-gradient"
      >
        <div>
          <span className="inline-block text-xs font-extrabold tracking-widest uppercase text-brand-200 bg-white/10 px-3 py-1 rounded-full mb-4">
            New Arrivals 2026
          </span>
          <h1 className="text-4xl md:text-5xl font-black text-white leading-tight mb-4">
            Latest Trending{" "}
            <span className="text-brand-300">Electronic Items</span>
          </h1>
          <p className="text-indigo-200 text-sm mb-8 leading-relaxed max-w-sm">
            Discover the best deals on top electronics. Fast shipping, free returns on every order.
          </p>
          <div className="flex gap-3 flex-wrap">
            <Link
              to="/products"
              className="inline-block bg-brand-gradient text-white text-sm font-bold px-8 py-3.5 rounded-xl hover:opacity-85 transition-opacity"
            >
              Shop Now →
            </Link>
            <Link
              to="/products"
              className="inline-block bg-white/10 text-white text-sm font-bold px-8 py-3.5 rounded-xl border border-white/20 hover:bg-white/20 transition-all"
            >
              View All
            </Link>
          </div>
        ...
        <div className="flex justify-center items-center gap-4 flex-wrap">
          {loading
            ? [1, 2, 3].map((i) => (
                <div key={i} className="bg-white/10 rounded-2xl w-28 h-36 animate-pulse" />
              ))
